...
context("Testing knots functions") {
  test_that("'get_all_knots' yields correct result") {
    {
      ...
      auto out = get_all_knots(x, order);
  ...
  test_that("'get_all_knots' yields correct result") {
    ...
    auto out = get_knots_w_span(x, 2, 3, i);
    expect_true(out.knots.n_elem == 0);
    expect_true(out.n_unique == 1);

    x << 2 << 1 << 1 << 1 << 1 << 1 << 0;
    out = get_knots_w_span(x, 2, 3, i);
    arma::vec expect;
    expect << 1;
    ...
    out = get_knots_w_span(x, 1, 3, i);
    expect << 8 << 5 << 2 << 1;
    expect_true(is_all_equal(out.knots, expect));
    expect_true(out.n_unique == 10);

    out = get_knots_w_span(x, 2, 2, i);
    expect << 7 << 5 << 3 << 2;
    expect_true(is_all_equal(out.knots, expect));
    expect_true(out.n_unique == 10);

    out = get_knots_w_span(x, 4, 4, i);
    expect << 5 << 4;
    expect_true(is_all_equal(out.knots, expect));
    expect_true(out.n_unique == 10);
  }
