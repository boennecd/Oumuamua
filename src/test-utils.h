...
bool is_all_equal(T1 first1, T1 end1, T2 first2, T2 end2){
  auto d1 = std::distance(first1, end1), d2 = std::distance(first2, end2);
  if(d1 != d2)
    throw std::invalid_argument(
        "The length of iterators do not match (" + std::to_string(d1) + ", " + std::to_string(d2) + ")");
...
/* R function to get array to use
 dput_2_cpp <- function(x){
 wrap <- function(z)
 paste("{", paste0(z, collapse = ", "), "}")

 ty <- typeof(x)
 if(ty == "double"){
 x <- as.character(x)
 no_period <- which(!grepl("\\.", x))
 if(length(no_period) > 0L)
 x[no_period] <- paste0(x[no_period], ".")

 return(cat(wrap(x), "\n"))
 } else if(ty == "integer"){
 x <- paste0("(unsigned int)", x)
 return(cat(wrap(x), "\n"))
 }
 stop("Unkown type")
 }
