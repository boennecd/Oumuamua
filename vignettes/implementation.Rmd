...
update_normal_equation <- function(
  ...
  z_old[idx_new] <- z_old[idx_new] + 
    ...
    (knot_old - knot_new) * sum(y_cen[active] * parent[active])
  ...
  Vc_new[idx_new] <-  Vc_new[idx_new] + 
    ...
    (knot_old - knot_new) * sum(parent[active]^2 * (
...
1. center and scale covariates and center outcomes. Store the means and 
...
2. Using 1., get the ordered design matrix, ordered outcome, and ordered
   covariate value. This requires copies but this will be presumably be made 
   up for by avoiding (many) cache misses later.
3. Compute the knots to consder [@friedman91, pp. 26-28].
